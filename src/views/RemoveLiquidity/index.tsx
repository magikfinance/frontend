...
import { Currency, currencyEquals, ETHER, Percent, WNATIVE } from '@magikswap/sdk'
import {
  Button,
  Text,
  AddIcon,
                  ...
                  {chainId && (oneCurrencyIsWBNB || oneCurrencyIsBNB) ? (
                    <RowBetween style={{ justifyContent: 'flex-end', fontSize: '14px' }}>
                      {oneCurrencyIsBNB ? (
                        ...
                        <StyledInternalLink
                          href={`/remove/${
                            currencyA && currencyEquals(currencyA, WNATIVE[chainId]) ? 'FTM' : currencyIdA
                          }/${currencyB && currencyEquals(currencyB, WNATIVE[chainId]) ? 'FTM' : currencyIdB}`}
                        >
                          {t('Receive FTM')}
                        </StyledInternalLink>
                      ) : null}
